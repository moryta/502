...
class Lab2
{
	...
	public function converterHorario($fuso)
	{
	    ...
	    $horarioLocal = $dataLocal->format('d-m-Y H:i:s');

	    $fusoHorario = new DateTimeZone($fuso);
	    $dataLocal->setTimezone($fusoHorario);

	    $horarioFuso = $dataLocal->format('d-m-Y H:i:s');

	    // Modelo de array que deverá ser retornado
		$retorno = array(
		    'horario_brasil'   => $horarioLocal,
			'fuso_escolhido'   => $fuso,
		    'horario'          => $horarioFuso,
		);
